...
const CONTENT_KEY = Buffer.from(
  "0a237b0752cbf1a827e2fecfb87479a2",
  "hex"
).toString("base64");
const KEY_ID = Buffer.from("d58ce954203b7c9a9a9d467f59839249", "hex").toString(
  "base64"
);
...
function processLicenseResponse(response) {
  ...
  if (license_response.status == "OK") {
    if (license_response.license)
      return {
        status_ok: true,
        response: Buffer.from(license_response.license, "base64")
      };
...
app.post("/", async (req, res) => {
  ...
  try {
    if (sizeof(Base64.encode(body)) < 50) {
      response = await sendRequest(buildCertificateRequest(body));
    } else {
      response = await sendRequest(buildLicenseServerRequest(body));
    }
